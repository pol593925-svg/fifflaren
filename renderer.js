...
document.addEventListener("DOMContentLoaded", () => {
  // 0. Логика модального окна авторизации (Штаб / Ник)
  const savedUser = localStorage.getItem('support_hub_user');
  const loginModal = document.getElementById('login-modal');

  function applyUserToInputs(username) {
    localStorage.setItem('global_worker_nick', username);
    
    const globalNickInput = document.getElementById("globalWorkerNick");
    if (globalNickInput) globalNickInput.value = username;

    const shiftNick = document.getElementById("userNick");
    if (shiftNick && !shiftNick.disabled) shiftNick.value = username;

    // Обновляем статус онлайн в штабе
  ...
  if (!savedUser) {
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', () => {
        ...
        if (selectedUser) {
          localStorage.setItem('support_hub_user', selectedUser);
          if (loginModal) loginModal.style.display = 'none';
          applyUserToInputs(selectedUser);
          // Запускаем пинг сразу после авторизации
          initRealtimeStatus();
        } else {
          alert('Пожалуйста, выберите свой ник из списка!');
        }
      });
    }
  } else {
    if (loginModal) loginModal.style.display = 'none';
    applyUserToInputs(savedUser);
  }

  // 1. Переключение вкладок (включая вкладку "Штаб")
  ...
  // 2. Загрузка сохраненных данных смен и заметок
  try {
  ...
  // 4. Запуск проверки онлайн-статусов реального времени
  setTimeout(initRealtimeStatus, 1000);
  setInterval(initRealtimeStatus, 15000);
});

// Функция для подсветки активного пользователя в штабе (локальная базовая)
...
// Новая логика получения и отображения реального онлайна с сервера
function initRealtimeStatus() {
  ...
  // 1. Отправляем сигнал heartbeat в таблицу
  fetch(WEB_APP_URL, {
    method: "POST",
    mode: "no-cors",
  ...
  // 2. Запрашиваем список активных пользователей из таблицы
  fetch(WEB_APP_URL)
    .then(res => res.json())
    .then(onlineNicks => {
      updateOnlineUI(onlineNicks, currentNick);
    })
    .catch(err => console.error("Ошибка получения онлайн-статусов:", err));
}

function updateOnlineUI(onlineNicks, currentNick) {
  const items = document.querySelectorAll('.team-list li');
  items.forEach(item => {
    const nickName = item.getAttribute('data-nick');
    if (!nickName) return;

    // Проверяем, есть ли ник в списке онлайн от сервера
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedNick = localStorage.getItem('global_worker_nick');

  if (savedNick) {
    document.getElementById("globalWorkerNick").value = savedNick;
    const shiftNick = document.getElementById("userNick");
    if (shiftNick) shiftNick.value = savedNick;
  }
  ...
  // 3. Привязка кнопок к функциям
  document.getElementById("globalWorkerNick").addEventListener("input", saveGlobalNick);
  document.getElementById("templateBtn").addEventListener("click", insertTruffleTemplate);
  document.getElementById("sendBugBtn").addEventListener("click", sendBugReport);
  document.getElementById("startBtn").addEventListener("click", toggleShift);
  document.getElementById("stopBtn").addEventListener("click", finishShift);
...
function sendBugReport() {
  const nick = document.getElementById("globalWorkerNick").value.trim() || "Аноним";
  const type = document.getElementById("bugType").value;
  const desc = document.getElementById("bugDesc").value.trim();
  if (!desc) { alert("Заполните описание!"); return; }
...
function sendOvertime() {
  const nick = document.getElementById("globalWorkerNick").value.trim() || document.getElementById("userNick").value.trim() || "Сотрудник";
  const fromTime = document.getElementById("overtimeFrom").value;
  const toTime = document.getElementById("overtimeTo").value;
